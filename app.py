...
st.markdown("""
    ...
    <p style="font-size:14px; color:gray;">실시간 시세, 등락률, 포트폴리오 계산기, 그래프를 함께 제공합니다.</p>
...
graph_market = st.selectbox(
    "📊 차트 표시할 코인을 선택하세요:",
    options=selected_markets if selected_markets else ["선택된 코인 없음"],
    ...
    while True:
        ...
        with left_col:
            if graph_market:
                ...
                if graph_data:
                    price_logs[graph_market].append({"시간": now, "가격": graph_data["trade_price"]})
                    if len(price_logs[graph_market]) > 30:
                        price_logs[graph_market].pop(0)

                    df = pd.DataFrame(price_logs[graph_market])
                    st.markdown(f"<h4 style='font-size:18px;'>{markets_dict[graph_market]} 차트</h4>", unsafe_allow_html=True)
        ...
        with right_col:
            for i, market in enumerate(selected_markets):
                ...
                if data:
                    ...
                    qty = st.number_input(
                        f"{coin_name} 보유 수량", min_value=0.0,
                        value=float(holdings[market]), step=0.01,
